...
def get_hexbug_points(hexbugs: list) -> list:
    """
...
def get_center_points(hexbug_points: list) -> np.ndarray:
...
def get_frame_masks(
    ...
    Args:
        sam2_predictor (SAM2ImagePredictor): The predictor object used to generate masks from input points.
    ...
    """
    hexbug_points = get_hexbug_points(csv_data[frame_index])
    center_points = get_center_points(hexbug_points)
    # SAM
    ...
    return [
        {"mask": mask, "x": hexbug["x"], "y": hexbug["y"], "hexbug": hexbug["hexbug"]}
...
if __name__ == "__main__":
    ...
    # Directories
    CSV_DIR_PATH = "./traco_2024/training"
    FRAMES_DIR = "./data/frames"
    OUTPUT_DIR = "./data/annotated_frames_body"
    YOLO_DIR = "./data/yolo_labels_body"
    ...
    all_videos = sorted(os.listdir(FRAMES_DIR))
    for video_name in problematic_videos:
        print(f"Processing video: {video_name}")
        frame_path = os.path.join(FRAMES_DIR, video_name)
        csv_path = os.path.join(CSV_DIR_PATH, f"{video_name}.csv")
        yolo_data = []
        missing_data = []
        ...
        # Save YOLO labels
        yolo_label_path = os.path.join(YOLO_DIR, f"{video_name}.txt")
        with open(yolo_label_path, "w", encoding="utf-8") as f:
            for i, row in enumerate(yolo_data):
                f.write(
